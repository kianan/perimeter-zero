# QA Report — TICKET-008: Enemy scale tiers 2-5 (rusher/swarm/tank)

**Method:** static code review

**Verdict:** PASS

**Summary:** Checked public/data/enemy_scale.csv against TICKET-008 criteria: 15 data rows (5 each for rusher/swarm/tank) + header, tiers 1-5, no level >5. Tier 1 values for all three (rusher 10/20, swarm 5/10, tank 15/50) match the pre-existing baseline. Verified compounding: each tier's damage/health = prior tier x1.25 rounded (e.g. rusher 10→13→16→20→25 dmg, 20→25→31→39→49 hp; swarm 5→6→8→10→13 dmg; tank 15→19→24→30→38 dmg, 50→63→79→99→124 hp) — all consistent with the round-half-up pattern shown in the brief. No discrepancies found.

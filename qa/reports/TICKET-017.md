# QA Report — TICKET-017: GameScene wiring: spawn Shooter/Charger with real behavior + enemy weapon fire

**Method:** static code review

**Verdict:** PASS

**Summary:** Static review of GameScene/Enemy/enemies.ts/archetypes.ts/CSVs against TICKET-017 criteria: (1) spawnEnemy() passes preferredRange/dashBurstMult/dashBurstMs/dashCooldownMs from getEnemy() into the Enemy ctor and gates startDashCycle() on dashBurstMult>0 — confirmed. (2) fireEnemyWeapon(enemy) exists, gated by `if (stats.weapon)` (non-blank FK), spawns Bullets aimed via Phaser.Math.Angle.Between(enemy,player) recalculated each tick, interval = 1000/weapon.fireRate (weapon id 2, weapon_scale fire_rate=1.2 → ~833ms), pushed into enemyBullets via trackEnemyBullet, and the fire callback no-ops once enemy.isDying() (timer itself cleaned up on DESTROY). (3) Shooter preferredRange=350 (enemy.csv) makes chase() hold station well outside the 110x150 contact body, and enemy_scale.csv shooter damage=0, so contact overlap (if any) deals 0 HP. (4) player-vs-enemyBullets overlap destroys the bullet and applies weapon damage (8) on hit; Bullet self-destroys via delayedCall(lifespanMs). (5) Charger dashBurstMult=1.8/dashBurstMs=400/dashCooldownMs=2200 (enemy.csv) drives startDashCycle(), which multiplies this.speed (read live by chase() every frame via pbody.setVelocity), so __qaGame-inspectable velocity magnitude rises during the burst window then reverts. (6) level_enemies.csv stage-1 rows include shooter (5000/1) and charger (5500/1). (7) rusher/swarm/tank all still pass preferredRange=0/dashBurstMult=0 (unchanged CSV values), so chase()'s early-return and startDashCycle() gating leave their behavior untouched — no regression. No logic defects found in the diff; tsc mechanical check reported clean.

# QA Report — TICKET-016: Enemy.ts: preferred-range stand-off + dash-burst cycle

**Method:** static code review

**Verdict:** PASS

**Summary:** Checked Enemy.ts against TICKET-016: EnemyStats/Enemy ctor takes preferredRange/dashBurstMult/dashBurstMs/dashCooldownMs as optional fields, defaulted to 0 via `?? 0` in the constructor. chase() prepends a `preferredRange > 0 && dist <= preferredRange` stand-off check (zero velocity + idle anim) before the original distance-based move/idle logic, which is untouched and still reached unconditionally when preferredRange===0 (0>0 short-circuits false) — equivalence holds. startDashCycle() arms `scene.time.addEvent({delay: dashCooldownMs, loop: true, ...})`, sets a distinct DASH_TINT (separate from the 0xff3b3b hit-flash color), multiplies `this.speed` off a stored `baseSpeed` by `dashBurstMult` for `dashBurstMs` via delayedCall, then reverts tint and speed. GameScene.spawnEnemy() still constructs Enemy with only {hp, speed, damage, scale} for rusher/swarm/tank, and nothing anywhere calls startDashCycle() — existing archetypes are unaffected, matching the ticket's explicit 'not wired yet' scope. Mechanical check reports tsc clean. No defects found against the stated criteria.
